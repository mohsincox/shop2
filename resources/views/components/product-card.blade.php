...
<div class="group relative overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
    ...
    @if ($product->featured)
        <span class="absolute left-3 top-3 rounded-full bg-indigo-600 px-2.5 py-1 text-[11px] font-bold uppercase tracking-wide text-white shadow">{{ __("Featured") }}</span>
    @elseif (!$product->isInStock())
        <span class="absolute left-3 top-3 rounded-full bg-rose-600 px-2.5 py-1 text-[11px] font-bold uppercase tracking-wide text-white shadow">{{ __("Sold out") }}</span>
    @endif
