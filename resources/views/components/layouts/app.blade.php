...
<body class="h-full bg-slate-50 font-sans text-slate-800 antialiased">
    <div class="flex min-h-full flex-col">
        ...
        <div class="bg-indigo-600 px-4 py-2 text-center text-xs font-semibold text-white sm:text-sm">
            {{ __('Free shipping on orders over $100') }} &nbsp;·&nbsp; <span class="opacity-80">{{ __('New season collection just dropped') }}</span>
        </div>
        ...
        <header class="sticky top-0 z-40 border-b border-slate-200/80 bg-white/90 backdrop-blur">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div class="flex h-16 items-center justify-between gap-4">
                    ...
                    <nav class="hidden items-center gap-1 lg:flex">
                        <a href="{{ route('home') }}" class="rounded-lg px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100 hover:text-slate-900">{{ __('Home') }}</a>
                        <a href="{{ route('shop.index') }}" class="rounded-lg px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100 hover:text-slate-900">{{ __('Shop') }}</a>
                        @auth
                            <a href="{{ route('orders.index') }}" class="rounded-lg px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100 hover:text-slate-900">{{ __('My Orders') }}</a>
                            @if (auth()->user()->isAdmin())
                                <a href="{{ route('admin.dashboard') }}" class="rounded-lg px-3 py-2 text-sm font-medium text-indigo-600 hover:bg-indigo-50">{{ __('Admin Panel') }}</a>
                            @endif
                        @endauth
                    </nav>
                    ...
                    <div class="flex items-center gap-1.5">
                        ...
                        <div class="relative" data-dropdown>
                            <button type="button" data-dropdown-toggle
                                class="inline-flex h-10 items-center gap-1.5 rounded-lg px-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-100"
                            ...
                            <div data-dropdown-menu class="absolute right-0 top-12 z-50 hidden w-40 overflow-hidden rounded-xl border border-slate-200 bg-white py-1.5 shadow-lg">
                        ...
                        <a href="{{ route('cart.index') }}" class="relative inline-flex h-10 w-10 items-center justify-center rounded-lg text-slate-600 hover:bg-slate-100" aria-label="{{ __('Your cart') }}">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor" class="h-6 w-6">
                        ...
                        @else
                            <a href="{{ route('login') }}" class="hidden rounded-full border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50 sm:inline-flex">{{ __('Sign in') }}</a>
                            <a href="{{ route('register') }}" class="hidden rounded-full bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-700 sm:inline-flex">{{ __('Sign up') }}</a>
                        @endauth
                    </div>
                </div>
            </div>
            ...
            <div data-mobile-menu class="hidden border-t border-slate-100 bg-white lg:hidden">
                <div class="space-y-1 px-4 py-3">
                    <a href="{{ route('home') }}" class="block rounded-lg px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100">{{ __('Home') }}</a>
                    <a href="{{ route('shop.index') }}" class="block rounded-lg px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100">{{ __('Shop') }}</a>
                    @auth
                        <a href="{{ route('orders.index') }}" class="block rounded-lg px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100">{{ __('My Orders') }}</a>
                        @if (auth()->user()->isAdmin())
                            <a href="{{ route('admin.dashboard') }}" class="block rounded-lg px-3 py-2 text-sm font-medium text-indigo-600 hover:bg-indigo-50">{{ __('Admin Panel') }}</a>
                        @endif
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit" class="block w-full rounded-lg px-3 py-2 text-left text-sm font-medium text-rose-600 hover:bg-rose-50">{{ __('Sign out') }}</button>
                        </form>
                    @else
                        <a href="{{ route('login') }}" class="block rounded-lg px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100">{{ __('Sign in') }}</a>
                        <a href="{{ route('register') }}" class="block rounded-lg px-3 py-2 text-sm font-medium text-indigo-600 hover:bg-indigo-50">{{ __('Sign up') }}</a>
                    @endauth
                ...
                <div class="grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
                    <div>
                        ...
                        <p class="mt-4 text-sm leading-relaxed text-slate-400">
                            {{ __('Your one-stop destination for quality products at unbeatable prices. Shop the latest trends today.') }}
                        </p>
                    </div>
                    <div>
                        <h3 class="text-sm font-semibold uppercase tracking-wider text-white">{{ __('Shop') }}</h3>
                        <ul class="mt-4 space-y-2.5 text-sm">
                            <li><a href="{{ route('shop.index') }}" class="hover:text-white">{{ __('All Products') }}</a></li>
                            <li><a href="{{ route('shop.index', ['sort' => 'price_asc']) }}" class="hover:text-white">{{ __('Best Deals') }}</a></li>
                            <li><a href="{{ route('shop.index', ['sort' => 'latest']) }}" class="hover:text-white">{{ __('New Arrivals') }}</a></li>
                        </ul>
                    </div>
                    <div>
                        <h3 class="text-sm font-semibold uppercase tracking-wider text-white">{{ __('Account') }}</h3>
                        <ul class="mt-4 space-y-2.5 text-sm">
                            @auth
                                <li><a href="{{ route('orders.index') }}" class="hover:text-white">{{ __('My Orders') }}</a></li>
                                <li><a href="{{ route('cart.index') }}" class="hover:text-white">{{ __('Your cart') }}</a></li>
                            @else
                                <li><a href="{{ route('login') }}" class="hover:text-white">{{ __('Sign in') }}</a></li>
                                <li><a href="{{ route('register') }}" class="hover:text-white">{{ __('Create account') }}</a></li>
                            @endauth
                        </ul>
                    </div>
                    <div>
