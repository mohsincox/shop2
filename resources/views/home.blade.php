<x-layouts.app>
        ...
        <div class="mx-auto grid max-w-7xl gap-10 px-4 py-20 sm:px-6 lg:grid-cols-2 lg:items-center lg:px-8 lg:py-28">
            <div class="relative">
                <span class="inline-flex items-center gap-2 rounded-full bg-white/15 px-4 py-1.5 text-xs font-semibold uppercase tracking-widest backdrop-blur">
                    <span class="h-2 w-2 animate-pulse rounded-full bg-emerald-300"></span> {{ __("Summer Sale · Up to 40% off") }}
                </span>
                <h1 class="mt-6 text-4xl font-extrabold leading-tight tracking-tight sm:text-5xl lg:text-6xl">
                    {{ __("Discover the best products, at the best prices") }}
                </h1>
                <p class="mt-6 max-w-xl text-lg text-indigo-100">
                    {{ __("From everyday essentials to premium finds — explore our carefully curated catalog and enjoy fast, free delivery on orders over $100.") }}
                </p>
                ...
                <div class="mt-12 flex flex-wrap gap-10">
                    <div>
                        ...
                        <p class="text-sm text-indigo-200">{{ __("Happy customers") }}</p>
                    </div>
                    <div>
                        <p class="text-3xl font-extrabold">500+</p>
                        <p class="text-sm text-indigo-200">{{ __("Products") }}</p>
                    </div>
                    <div>
                        <p class="text-3xl font-extrabold">24h</p>
                        <p class="text-sm text-indigo-200">{{ __("Fast delivery") }}</p>
                    </div>
                </div>
            </div>
            <div class="relative hidden lg:block">
                ...
                <div class="absolute -bottom-6 left-1/2 flex -translate-x-1/2 items-center gap-3 rounded-2xl bg-white px-6 py-4 shadow-2xl">
                    ...
                    <div>
                        <p class="text-sm font-bold text-slate-900">{{ __("Free shipping") }}</p>
                        <p class="text-xs text-slate-500">{{ __("On orders over $100") }}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <section class="border-b border-slate-200 bg-white">
        <div class="mx-auto grid max-w-7xl grid-cols-2 gap-6 px-4 py-8 sm:px-6 lg:grid-cols-4 lg:px-8">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-sm font-bold text-slate-900">{{ __("Secure payment") }}</p>
                    <p class="text-xs text-slate-500">{{ __("256-bit SSL encrypted") }}</p>
                </div>
            </div>
            <div class="flex items-center gap-3">
                <span class="flex h-12 w-12 items-center justify-center rounded-2xl bg-blue-50 text-2xl">↩️</span>
                <div>
                    <p class="text-sm font-bold text-slate-900">{{ __("Easy returns") }}</p>
                    <p class="text-xs text-slate-500">{{ __("30-day return policy") }}</p>
                </div>
            </div>
            <div class="flex items-center gap-3">
                <span class="flex h-12 w-12 items-center justify-center rounded-2xl bg-amber-50 text-2xl">💎</span>
                <div>
                    <p class="text-sm font-bold text-slate-900">{{ __("Premium quality") }}</p>
                    <p class="text-xs text-slate-500">{{ __("Hand-picked products") }}</p>
                </div>
            </div>
            <div class="flex items-center gap-3">
                <span class="flex h-12 w-12 items-center justify-center rounded-2xl bg-violet-50 text-2xl">🎧</span>
                <div>
                    <p class="text-sm font-bold text-slate-900">{{ __("24/7 support") }}</p>
                    <p class="text-xs text-slate-500">{{ __("Always here to help") }}</p>
                </div>
            </div>
        </div>
    </section>
    ...
    @if ($categories->isNotEmpty())
        <section class="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
            <div class="flex items-end justify-between">
                <div>
                    <p class="text-sm font-bold uppercase tracking-widest text-indigo-600">{{ __("Browse by category") }}</p>
                    <h2 class="mt-2 text-3xl font-extrabold tracking-tight text-slate-900">{{ __("Shop by category") }}</h2>
                </div>
                <a href="{{ route('shop.index') }}" class="hidden text-sm font-semibold text-indigo-600 hover:text-indigo-700 sm:block">{{ __("View all") }} →</a>
            </div>
                ...
                @foreach ($categories as $category)
                    <a href="{{ route('shop.index', ['category' => $category->slug]) }}"
                        ...
                        <div>
                            ...
                            <p class="text-xs text-slate-500">{{ $category->products_count }} {{ __("items") }}</p>
                        </div>
                    </a>
                @endforeach
            </div>
    ...
    @if ($featured->isNotEmpty())
        <section class="bg-white py-16">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div class="flex items-end justify-between">
                    <div>
                        <p class="text-sm font-bold uppercase tracking-widest text-indigo-600">{{ __("Hand-picked") }}</p>
                        <h2 class="mt-2 text-3xl font-extrabold tracking-tight text-slate-900">{{ __("Featured products") }}</h2>
                    </div>
                    <a href="{{ route('shop.index') }}" class="text-sm font-semibold text-indigo-600 hover:text-indigo-700">{{ __("View all") }} →</a>
                </div>
    ...
    <section class="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <div class="relative overflow-hidden rounded-3xl bg-gradient-to-r from-slate-900 via-slate-800 to-indigo-900 px-8 py-14 text-white sm:px-14">
            ...
            <div class="relative flex flex-col items-start justify-between gap-8 lg:flex-row lg:items-center">
                <div>
                    <p class="text-sm font-bold uppercase tracking-widest text-amber-400">{{ __("Limited time offer") }}</p>
                    <h2 class="mt-3 text-3xl font-extrabold tracking-tight sm:text-4xl">{{ __("New season, new favorites") }}</h2>
                    <p class="mt-3 max-w-lg text-slate-300">{{ __("Refresh your style with the latest arrivals. Members get free shipping on every order.") }}</p>
                </div>
    ...
    @if ($newArrivals->isNotEmpty())
        <section class="mx-auto max-w-7xl px-4 pb-8 sm:px-6 lg:px-8">
            <div class="flex items-end justify-between">
                <div>
                    <p class="text-sm font-bold uppercase tracking-widest text-indigo-600">{{ __("Just in") }}</p>
                    <h2 class="mt-2 text-3xl font-extrabold tracking-tight text-slate-900">{{ __("New arrivals") }}</h2>
                </div>
                <a href="{{ route('shop.index', ['sort' => 'latest']) }}" class="text-sm font-semibold text-indigo-600 hover:text-indigo-700">{{ __("View all") }} →</a>
            </div>
            <div class="mt-8 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
    ...
    <section class="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <div class="rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm sm:p-12">
            <p class="text-3xl">💌</p>
            <h2 class="mt-3 text-2xl font-extrabold tracking-tight text-slate-900">{{ __("Stay in the loop") }}</h2>
            <p class="mx-auto mt-2 max-w-md text-sm text-slate-500">{{ __("Subscribe to our newsletter and get 10% off your first order, plus exclusive offers.") }}</p>
            <form class="mx-auto mt-6 flex max-w-md gap-2" onsubmit="event.preventDefault(); this.querySelector('input').value=''; this.querySelector('button').textContent='{{ __("Subscribed ✓") }}';">
                @csrf
