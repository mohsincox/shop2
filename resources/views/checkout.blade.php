<x-layouts.app title="Checkout">
    <div class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <h1 class="mt-3 text-3xl font-extrabold tracking-tight text-slate-900">Checkout</h1>
        ...
        <form method="POST" action="{{ route('checkout.store') }}" class="mt-8 grid gap-8 lg:grid-cols-[1fr_400px]">
            ...
            <div class="space-y-6">
                ...
                <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <div class="mt-5 grid gap-4 sm:grid-cols-2">
                        ...
                        <div>
                            <label for="phone" class="mb-1.5 block text-sm font-semibold text-slate-700">Phone <span class="text-slate-400">(optional)</span></label>
                ...
                <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    <div class="flex items-center gap-3">
                        ...
                        <h2 class="text-lg font-extrabold text-slate-900">Shipping address</h2>
                    </div>
                    <div class="mt-5 grid gap-4 sm:grid-cols-2">
                        <div class="sm:col-span-2">
                            <label for="address" class="mb-1.5 block text-sm font-semibold text-slate-700">Street address</label>
                        ...
                        <div>
                            <label for="zip" class="mb-1.5 block text-sm font-semibold text-slate-700">ZIP / Postal code</label>
                ...
                <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <div class="mt-5 grid gap-3 sm:grid-cols-2">
                        <label class="flex cursor-pointer items-center gap-3 rounded-xl border border-slate-200 p-4 transition has-[:checked]:border-indigo-500 has-[:checked]:bg-indigo-50">
                            ...
                            <span>
                                <span class="block text-sm font-bold text-slate-900">Cash on Delivery</span>
                        ...
                        <label class="flex cursor-pointer items-center gap-3 rounded-xl border border-slate-200 p-4 transition has-[:checked]:border-indigo-500 has-[:checked]:bg-indigo-50">
                            ...
                            <span>
                                <span class="block text-sm font-bold text-slate-900">Credit / Debit Card</span>
                                <span class="block text-xs text-slate-500">💳 Visa · Mastercard</span>
                            </span>
                        </label>
                    </div>
                    @if (old('payment_method') === 'card' || request()->is('checkout'))
                        <div data-card-fields class="mt-4 grid gap-4 sm:grid-cols-2">
                            <div class="sm:col-span-2">
                                <label for="card_number" class="mb-1.5 block text-sm font-semibold text-slate-700">Card number</label>
                            ...
                            <div>
                                <label for="card_expiry" class="mb-1.5 block text-sm font-semibold text-slate-700">Expiry</label>
                                <input type="text" id="card_expiry" placeholder="MM/YY" readonly
                                    class="w-full cursor-not-allowed rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm text-slate-400">
                            </div>
                            <div>
                                <label for="card_cvc" class="mb-1.5 block text-sm font-semibold text-slate-700">CVC</label>
                                <input type="text" id="card_cvc" placeholder="123" readonly
                                    class="w-full cursor-not-allowed rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm text-slate-400">
                            </div>
                            <p class="sm:col-span-2 text-xs text-slate-400">💡 Demo store — card payments are simulated. Please use Cash on Delivery.</p>
                        </div>
                    @endif
                </div>
                ...
                {{-- Notes --}}
                <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    <label for="notes" class="mb-1.5 block text-sm font-semibold text-slate-700">Order notes <span class="text-slate-400">(optional)</span></label>
            ...
            <div class="h-fit rounded-2xl border border-slate-200 bg-white p-6 shadow-sm lg:sticky lg:top-24">
                <h2 class="text-lg font-extrabold text-slate-900">Your order</h2>
                    ...
                    @foreach ($items as $line)
                        <li class="flex items-center gap-3 py-3">
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-xs text-slate-500">${{ number_format($line['unit_price'], 2) }} each</p>
                            </div>
                ...
                <dl class="mt-4 space-y-3 border-t border-slate-200 pt-4 text-sm">
                    ...
                    <div class="flex justify-between">
                        <dt class="text-slate-500">Tax (8%)</dt>
                ...
                <button type="submit"
                    ...
                    Place order · ${{ number_format($subtotal + ($subtotal >= 100 ? 0 : 5.99) + $tax, 2) }}
                </button>
                <p class="mt-4 text-center text-xs text-slate-400">🔒 Secure checkout · You won't be charged until dispatch</p>
            </div>
        </form>
    </div>
</x-layouts.app>
