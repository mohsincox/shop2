        ...
        <div class="relative w-full max-w-md">
            <div class="overflow-hidden rounded-3xl border border-slate-200 bg-white/90 shadow-2xl backdrop-blur">
                <div class="bg-gradient-to-br from-indigo-600 to-violet-700 px-8 py-8 text-center">
                    ...
                    <h1 class="mt-4 text-2xl font-extrabold text-white">{{ __("Welcome back") }}</h1>
                    <p class="mt-1 text-sm text-indigo-100">{{ __("Sign in to continue shopping with us") }}</p>
                </div>
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-5 px-8 py-8">
                    ...
                    <p class="text-center text-sm text-slate-500">
                        ...
                        <a href="{{ route('register') }}" class="font-semibold text-indigo-600 hover:text-indigo-700">{{ __("Create one") }}</a>
                    </p>
                </form>
            </div>
