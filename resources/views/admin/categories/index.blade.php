<x-layouts.admin :title="__('Categories')">
    <div class="grid gap-8 lg:grid-cols-[360px_1fr]">
        {{-- {{ __("Add category") }} form --}}
        <div class="h-fit rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <h2 class="text-lg font-extrabold text-slate-900">{{ __("Add category") }}</h2>
            <form method="POST" action="{{ route('admin.categories.store') }}" class="mt-5 space-y-4">
                @csrf
                <div>
                    <label for="name" class="mb-1.5 block text-sm font-semibold text-slate-700">{{ __("Category name") }}</label>
                ...
                <div>
                    <label for="description" class="mb-1.5 block text-sm font-semibold text-slate-700">{{ __("Description") }} <span class="font-normal text-slate-400">(optional)</span></label>
        ...
        <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div class="border-b border-slate-100 px-6 py-4">
                <h2 class="font-extrabold text-slate-900">{{ __("All categories") }}</h2>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-slate-100 bg-slate-50 text-left text-xs uppercase tracking-wider text-slate-400">
                            <th class="px-6 py-3.5 font-bold">{{ __("Name") }}</th>
                            <th class="px-6 py-3.5 font-bold">{{ __("Slug") }}</th>
                            <th class="px-6 py-3.5 font-bold">{{ __("Products") }}</th>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex justify-end gap-2">
                                        <button type="button" data-edit-category="{{ $category->id }}"
                                            ...
                                            class="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-bold text-slate-600 transition hover:border-indigo-300 hover:text-indigo-600">{{ __("Edit") }}</button>
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-16 text-center text-slate-400">
                                    <p class="text-4xl">&#128193;</p>
                                    <p class="mt-3 font-semibold">{{ __("No categories yet. Create your first one!") }}</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    <div data-edit-modal class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-900/50 p-4">
        <div class="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
            <h3 class="text-lg font-extrabold text-slate-900">{{ __("Edit") }} category</h3>
            <form method="POST" action="" data-edit-form class="mt-5 space-y-4">
                @csrf
                @method('PUT')
                <div>
                    <label class="mb-1.5 block text-sm font-semibold text-slate-700">{{ __("Category name") }}</label>
                ...
                <div>
                    <label class="mb-1.5 block text-sm font-semibold text-slate-700">{{ __("Description") }}</label>
                ...
                <div class="flex gap-3 pt-2">
                    <button type="submit" class="flex-1 rounded-xl bg-indigo-600 py-2.5 text-sm font-bold text-white transition hover:bg-indigo-700">{{ __("Save changes") }}</button>
                    <button type="button" data-edit-close class="rounded-xl border border-slate-300 px-6 py-2.5 text-sm font-bold text-slate-600 transition hover:bg-slate-50">{{ __("Cancel") }}</button>
                </div>
            </form>
        </div>
    </div>
