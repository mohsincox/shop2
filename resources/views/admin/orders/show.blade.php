<x-layouts.admin :title="__('Order').' '.$order->order_number">
    <nav class="text-sm text-slate-500">
        <a href="{{ route('admin.orders.index') }}" class="hover:text-indigo-600">&larr; {{ __("Back to orders") }}</a>
    </nav>

    <div class="mt-4 rounded-2xl bg-gradient-to-br from-slate-900 to-indigo-900 p-6 text-white">
        <div class="flex flex-wrap items-center justify-between gap-6">
            <div>
                <p class="text-xs font-bold uppercase tracking-widest text-slate-400">{{ __("Order") }}</p>
            ...
            <div class="rounded-2xl bg-white/10 px-5 py-4 backdrop-blur">
                <p class="text-xs font-bold uppercase tracking-widest text-slate-300">{{ __("Total") }}</p>
        ...
        <div class="mt-6 flex flex-wrap items-center gap-4 border-t border-white/10 pt-6">
            <span class="rounded-full px-3 py-1.5 text-xs font-bold bg-white/15">{{ $order->statusLabel() }}</span>
            <form method="POST" action="{{ route('admin.orders.update-status', $order) }}" class="flex items-center gap-2">
                ...
                <select name="status"
                    ...
                    @foreach (\App\Models\Order::STATUSES as $status)
                        <option value="{{ $status }}" @selected($order->status === $status)>{{ __(ucfirst($status)) }}</option>
                    @endforeach
                </select>
    ...
    <div class="mt-8 grid gap-8 lg:grid-cols-[1fr_340px]">
        ...
        <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div class="border-b border-slate-100 px-6 py-4">
                <h2 class="font-extrabold text-slate-900">{{ __("Order") }} items ({{ $order->items->count() }})</h2>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-slate-100 bg-slate-50 text-left text-xs uppercase tracking-wider text-slate-400">
                            <th class="px-6 py-3.5 font-bold">{{ __("Product") }}</th>
                            <th class="px-6 py-3.5 font-bold">{{ __("Price") }}</th>
                            <th class="px-6 py-3.5 font-bold">{{ __("Qty") }}</th>
        ...
        <div class="space-y-6">
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <h2 class="text-sm font-extrabold uppercase tracking-wide text-slate-900">{{ __("Customer") }}</h2>
                ...
                <div class="mt-5 space-y-1 text-sm text-slate-600">
                    <p class="font-semibold text-slate-800">{{ __("Shipping to:") }}</p>
                    <p>{{ $order->address }}, {{ $order->city }}{{ $order->zip ? ' '.$order->zip : '' }}</p>
                    <p class="mt-2 font-semibold text-slate-800">{{ __("Payment:") }} <span class="font-normal capitalize">{{ $order->payment_method }}</span></p>
                    @if ($order->notes)
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <h2 class="text-sm font-extrabold uppercase tracking-wide text-slate-900">{{ __("Summary") }}</h2>
