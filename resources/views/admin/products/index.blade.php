    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
        <div class="flex flex-wrap gap-3">
            <form action="{{ route('admin.products.index') }}" method="GET" class="flex gap-2">
                ...
                <button type="submit" class="rounded-xl bg-indigo-600 px-4 py-2 text-sm font-bold text-white hover:bg-indigo-700">{{ __("Filter") }}</button>
            </form>
        </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-slate-100 bg-slate-50 text-left text-xs uppercase tracking-wider text-slate-400">
                        ...
                        <th class="px-6 py-3.5 font-bold">{{ __("Category") }}</th>
                        <th class="px-6 py-3.5 font-bold">{{ __("Price") }}</th>
                        <th class="px-6 py-3.5 font-bold">{{ __("Stock") }}</th>
                        <th class="px-6 py-3.5 font-bold">{{ __("Featured") }}</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($products as $product)
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="px-6 py-3.5">
                                @if ($product->stock <= 5)
                                    <span class="font-bold text-amber-600">{{ $product->stock }} {{ __("left") }}</span>
                                @elseif ($product->stock > 0)
                                    <span class="font-bold text-emerald-600">{{ $product->stock }}</span>
                                @else
                                    <span class="font-bold text-rose-600">{{ __("Out of stock") }}</span>
                                @endif
                            </td>
                            <td class="px-6 py-3.5">
                                @if ($product->featured)
                                    <span class="rounded-full bg-amber-100 px-2.5 py-1 text-xs font-bold text-amber-700">&#9733; {{ __("Featured") }}</span>
                                @else
                            ...
                            <td class="px-6 py-3.5">
                                <div class="flex justify-end gap-2">
                                    <a href="{{ route('admin.products.edit', $product) }}"
                                        class="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-bold text-slate-600 transition hover:border-indigo-300 hover:text-indigo-600">{{ __("Edit") }}</a>
                                    <form method="POST" action="{{ route('admin.products.destroy', $product) }}"
                                        onsubmit="return confirm('{{ __("Delete this product permanently?") }}')">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-16 text-center text-slate-400">
                                <p class="text-4xl">&#128269;</p>
                                <p class="mt-3 font-semibold">{{ __("No products found") }}</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
