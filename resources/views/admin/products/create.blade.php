<x-layouts.admin :title="$product->exists ? 'Edit product' : 'Add product'">
    <div class="max-w-3xl">
        <div class="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
            <h2 class="text-xl font-extrabold text-slate-900">{{ $product->exists ? 'Edit product' : 'Create a new product' }}</h2>
            <p class="mt-1 text-sm text-slate-500">{{ $product->exists ? 'Update the details of '.$product->name.'.' : 'Fill in the details below to add a new product to your store.' }}</p>
            ...
            <form method="POST"
                ...
                <div class="grid gap-5 sm:grid-cols-2">
                    ...
                    <div>
                        <label for="price" class="mb-1.5 block text-sm font-semibold text-slate-700">Price ($)</label>
                    ...
                    <div>
                        <label for="stock" class="mb-1.5 block text-sm font-semibold text-slate-700">Stock quantity</label>
                ...
                <div>
                    <label for="image" class="mb-1.5 block text-sm font-semibold text-slate-700">Image URL <span class="font-normal text-slate-400">(optional — a placeholder is used if empty)</span></label>
                ...
                <label class="flex cursor-pointer items-center gap-3 rounded-xl border border-slate-200 p-4 transition has-[:checked]:border-indigo-500 has-[:checked]:bg-indigo-50">
                    ...
                    <span>
                        <span class="block text-sm font-bold text-slate-900">&#9733; Mark as featured</span>
                        <span class="block text-xs text-slate-500">Featured products appear on the homepage and get a discount badge.</span>
                    </span>
                </label>
                ...
                <div class="flex items-center gap-3 border-t border-slate-100 pt-6">
                    ...
                    <a href="{{ route('admin.products.index') }}" class="rounded-xl border border-slate-300 px-6 py-2.5 text-sm font-bold text-slate-600 transition hover:bg-slate-50">Cancel</a>
                </div>
            </form>
        </div>
    </div>
