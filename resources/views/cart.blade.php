<x-layouts.app :title="__('Shopping cart')">
    <div class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        <h1 class="text-3xl font-extrabold tracking-tight text-slate-900">{{ __("Shopping cart") }}</h1>
        <p class="mt-2 text-slate-500">{{ $count }} {{ $count === 1 ? __('item') : __('items') }} {{ __('in your cart') }}</p>
        ...
        @if ($items->isEmpty())
            <div class="mt-10 flex flex-col items-center rounded-3xl border border-dashed border-slate-300 bg-white py-24 text-center">
                ...
                <h2 class="mt-6 text-xl font-bold text-slate-900">{{ __("Your cart is empty") }}</h2>
                <p class="mt-2 max-w-sm text-sm text-slate-500">{{ __("Looks like you haven't added anything yet. Let's find something you'll love!") }}</p>
            ...
            <div class="mt-8 grid gap-8 lg:grid-cols-[1fr_360px]">
                ...
                <div class="space-y-4">
                        ...
                        <div class="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:flex-row sm:items-center">
                            ...
                            <div class="flex-1">
                                ...
                                @if ($line['quantity'] > $product->stock)
                                    <p class="mt-1 text-xs font-semibold text-rose-600">{{ __('Only') }} {{ $product->stock }} {{ __('left in stock') }}</p>
                                @endif
                            </div>
                            ...
                            <div class="flex items-center gap-4 sm:flex-col sm:items-end">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <form method="POST" action="{{ route('cart.destroy') }}">
                                        ...
                                        @method('DELETE')
                                        <input type="hidden" name="product_id" value="{{ $product->id }}">
                                        <button type="submit" class="rounded-full p-2 text-slate-400 transition hover:bg-rose-50 hover:text-rose-600" title="{{ __('Remove') }}">
                    ...
                    <div class="flex justify-end">
                        <a href="{{ route('shop.index') }}" class="text-sm font-semibold text-indigo-600 hover:text-indigo-700">← {{ __("Continue shopping") }}</a>
                    </div>
                </div>
                ...
                <div class="h-fit rounded-2xl border border-slate-200 bg-white p-6 shadow-sm lg:sticky lg:top-24">
                    ...
                    <dl class="mt-6 space-y-4 text-sm">
                        ...
                        <div class="flex justify-between">
                            <dt class="text-slate-500">{{ __("Estimated tax (8%)") }}</dt>
                        ...
                        @if ($subtotal < 100)
                            <div class="rounded-xl bg-amber-50 px-4 py-3 text-xs text-amber-700">
                                {{ __("Add :amount more to unlock free shipping 🚚", ['amount' => '$'.number_format(100 - $subtotal, 2)]) }}
                            </div>
                        @endif
                    ...
                    <p class="mt-4 text-center text-xs text-slate-400">{{ __("Secure checkout · SSL encrypted") }}</p>
                </div>
            </div>
        @endif
    </div>
