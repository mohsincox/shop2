<x-layouts.app title="My orders">
    <div class="mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <p class="mt-2 text-slate-500">Track and manage all your purchases.</p>
        ...
        @if ($orders->isEmpty())
            <div class="mt-10 flex flex-col items-center rounded-3xl border border-dashed border-slate-300 bg-white py-24 text-center">
                ...
                <h2 class="mt-6 text-xl font-bold text-slate-900">No orders yet</h2>
                <p class="mt-2 max-w-sm text-sm text-slate-500">When you place an order, it will show up here so you can track its status.</p>
                    ...
                    <a href="{{ route('orders.show', $order) }}" class="block rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:border-indigo-300 hover:shadow-md">
                        <div class="flex flex-wrap items-center justify-between gap-4">
                            <div>
                                ...
                                <p class="mt-0.5 text-xs text-slate-500">Placed on {{ $order->created_at->format('M d, Y · h:i A') }}</p>
                            </div>
                            <div class="flex items-center gap-4">
                                <span class="rounded-full px-3 py-1 text-xs font-bold {{ $order->statusColor() }}">{{ ucfirst($order->status) }}</span>
                                <span class="text-lg font-extrabold text-slate-900">${{ number_format($order->total, 2) }}</span>
                                <span class="text-xs text-slate-400">{{ $order->items_count }} {{ $order->items_count === 1 ? 'item' : 'items' }}</span>
                            </div>
                        </div>
                    </a>
                @endforeach
